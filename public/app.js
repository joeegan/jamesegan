...
function obtrusive(left, top) {
   return document.elementFromPoint(left, top).tagName === 'SPAN';
}

function getFreshPositions(){
   var left = rand(document.documentElement.clientWidth - 40);
   var top = rand(document.documentElement.clientHeight - 10);
   if (obtrusive(left, top)) {
      getFreshPositions();
   }
   return {
      left: left,
...
function convertToSpans(arr){
   arr.forEach(function(item, idx){
      ...
      span.innerHTML = item;
      var positions = getFreshPositions();
